...
(function () {
  ...
  function makeLayout(D) {
    ...
    if (!RANDOMIZE || chapterIdx < 1) return orig;                 // 첫 각성(1장)만 의도된 배치 고정
    ...
    // 매 진행마다 '모든 조각'(코어·에코·거짓·숨김)을 도달가능·간격 보장하며 재배치.
    //   obj 사물은 조각과 함께 그려지므로 위치가 같이 따라간다(서사-사물 페어 유지).
    const placed = [], shardsOut = D.shards.map(() => null);
    const pool = shuffle(reach.filter(t => d2(t, sp) >= 9));        // 스폰 근접(≤3타일) 제외 — 첫 행동이 함정/즉시수집이 되지 않게
  ...
  function loadStage(idx) {
    ...
    collectFx = null; paused = false; initAmbient();
    boss = D.boss ? { hp: D.boss.hp || 3, max: D.boss.hp || 3, x: px(D.boss.tile[0]), y: px(D.boss.tile[1]),
                      t: 0, hit: 0, dispelled: false, exitT: 0,
                      phase: "idle", timer: 1.8, waveR: (D.boss.waveR || 3.4) * TILE, ringT: 0 } : null;
  ...
  function gotoTitle() { hope = 0; paused = false; loadStage(0); phase = "title"; if (window.Audio2 && Audio2.music) try { Audio2.music("title"); } catch (e) {} }
  // 일시정지 메뉴(접근성 토글 포함)
  ...
  function firstGesture() { try { if (window.Audio2) { Audio2.init(); if (Audio2.setMuted) Audio2.setMuted(muted); if ((phase === "title" || phase === "select") && Audio2.music) Audio2.music("title"); } } catch (e) {} }  // 첫 제스처로 오디오 활성(+저장된 음소거) + 타이틀 BGM
  ...
  const pauseBtnRect = () => ({ x: cv.width - 190, y: 78, w: 34, h: 26 });   // 터치용 일시정지 버튼
  ...
  function onPointerDown(e) {
    ...
    if (phase === "play" && paused) {                               // 일시정지 메뉴 항목 탭
      for (let i = 0; i < pauseItems().length; i++) if (inRect(p, pauseRowRect(i))) { pauseSel = i; doPauseAction(pauseItems()[i].k); return; }
      return;
    }
    if (phase === "play" && inRect(p, pauseBtnRect())) { paused = true; if (window.Audio2 && Audio2.sfx) Audio2.sfx("select"); return; }
    if (phase === "title") {
  ...
  function tileOcc(tc, tr) { return D.collision.some(q => tc >= q[0] && tc < q[0]+q[2] && tr >= q[1] && tr < q[1]+q[3]); }
  function losClear(ax, ay, bx, by) {              // 시야 차단: 두 점 사이에 가구/벽 타일이 있으면 false
    const steps = Math.max(1, Math.ceil(dist(ax, ay, bx, by) / 6));
    for (let i = 1; i < steps; i++) { const x = ax + (bx-ax)*i/steps, y = ay + (by-ay)*i/steps;
      if (tileOcc(Math.floor(x/TILE), Math.floor(y/TILE))) return false; }
    return true;
  }
  ...
  function update(dt) {
    ...
    if (edge["escape"]) { paused = !paused; if (window.Audio2 && Audio2.sfx) Audio2.sfx("select"); }   // ESC 일시정지
    if (paused) {
      const items = pauseItems(), pre = pauseSel;
      if (edge["w"] || edge["arrowup"]) pauseSel = (pauseSel + items.length - 1) % items.length;
      if (edge["s"] || edge["arrowdown"]) pauseSel = (pauseSel + 1) % items.length;
      if (pauseSel !== pre && window.Audio2 && Audio2.sfx) Audio2.sfx("hover");
      if (edge[" "] || edge["enter"]) doPauseAction(items[pauseSel].k);
      clearEdges(); return;                                          // 일시정지 중 월드 정지
    }
    ...
    if (edge["q"]) {
      if (st.light >= 1) {
        ...
        if (boss && !boss.dispelled && boss.phase === "idle" && Core.chapterClear(st) && dist(player.x, player.y, boss.x, boss.y) <= AURA_R + 28) {
          boss.hp -= 1; boss.hit = 0.7; flash = 0.14; shake = 3; boss.timer = Math.min(boss.timer, 0.55);   // 반격(곧 파동) → 물러나야 함
          if (boss.hp <= 0) { boss.dispelled = true; boss.exitT = 1.4; toasts.push({ text: "공백이 흩어진다 — 빛만이 남는다", color: "#9af6f6", t: 2.4 }); }
          else toasts.push({ text: "기억의 빛이 공백을 갈라낸다  (" + (boss.max - boss.hp) + "/" + boss.max + ")  — 물러나라!", color: "#9af6f6", t: 2.0 });
        } else
    ...
    for (const m of murks) {
      ...
      const sees = !warded && Core.murkSees({ x: m.x, y: m.y, faceAngle: m.faceAngle, sight: m.sightPx * sm, fov: m.fov }, player.x, player.y)
                   && losClear(m.x, m.y, player.x, player.y);   // 가구 뒤에 숨으면 시야 차단(스텔스 깊이)
    ...
    if (st.mem <= 0 && Core.setbackIfDead(st)) {
      const z = safes[0];                                              // 안전지대 없으면(보스 등) 스폰으로 복귀(NaN 방지)
      if (z) { player.x = z.x + z.w/2; player.y = z.y + z.h/2; } else { player.x = px(D.spawn[0]); player.y = px(D.spawn[1]); }
    ...
    // 보스(공백) 상태기계 — idle(취약·비추기 가능) → charge(망각 파동 예고) → burst(근접 피해)
    if (boss) {
      boss.t += dt; if (boss.hit > 0) boss.hit -= dt; if (boss.ringT > 0) boss.ringT -= dt;
      if (!boss.dispelled) {
        boss.timer -= dt;
        if (boss.phase === "idle" && boss.timer <= 0) { boss.phase = "charge"; boss.timer = 0.85; }
        else if (boss.phase === "charge" && boss.timer <= 0) {           // 파동 터짐: 근접 시 기억 피해
          boss.phase = "idle"; boss.timer = 1.7; boss.ringT = 0.5;
          if (dist(player.x, player.y, boss.x, boss.y) < boss.waveR && projInvuln <= 0) {
            if (Core.contact(st)) { flash = 0.22; shake = 6; if (window.Audio2) Audio2.sfx("contact"); }
          } else if (window.Audio2 && Audio2.chime) Audio2.chime();      // 빗나감
        }
      }
  ...
  function draw() {
    ...
    ctx.imageSmoothingEnabled = false;
    const shk = reduceFlash ? 0 : Math.max(0, shake);   // 접근성: 화면 흔들림 감소
    const sx = (Math.random() - 0.5) * 2 * shk, sy = (Math.random() - 0.5) * 2 * shk;
    ...
    if (boss && !boss.dispelled) {
      ...
      if (ready && boss.phase === "idle") { ctx.globalAlpha = 0.4 + 0.45*pul; ctx.strokeStyle = "#9af6f6"; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(boss.x, boss.y, r+20, 0, 7); ctx.stroke(); }
      // 망각 파동 예고(charge): 보스 둘레 위험 반경이 붉게 차오름 → 이 안에 있으면 피해
      if (boss.phase === "charge") {
        const k = 1 - boss.timer / 0.85;
        ctx.globalAlpha = 0.18 + 0.22 * k; ctx.fillStyle = "#ff5a5a";
        ctx.beginPath(); ctx.arc(boss.x, boss.y, boss.waveR, 0, 7); ctx.fill();
        ctx.globalAlpha = 0.5 + 0.4 * Math.sin(boss.t * 30); ctx.strokeStyle = "#ff7a7a"; ctx.lineWidth = 2;
        ctx.beginPath(); ctx.arc(boss.x, boss.y, boss.waveR, 0, 7); ctx.stroke();
      }
      // 파동 터짐 링(밖으로 확장)
      if (boss.ringT > 0) { const e = 1 - boss.ringT / 0.5; ctx.globalAlpha = boss.ringT / 0.5 * 0.7;
        ctx.strokeStyle = "#ff5a5a"; ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(boss.x, boss.y, boss.waveR * (0.7 + 0.8*e), 0, 7); ctx.stroke(); }
    ...
    // 피격 플래시
    if (flash > 0 && !reduceFlash) { ctx.fillStyle = `rgba(255,90,90,${Math.min(0.5, flash)})`; ctx.fillRect(0, 0, NW, NH); }

    ctx.restore();
    if (phase === "play" || phase === "recall") drawAmbient();
    drawHUD();
    if (phase === "play") { drawGuide(); if (stageCardT > 0) overlayStageCard(); if (paused) overlayPause(); }
  ...
  function drawHUD() {
    ...
    // 단축키 안내는 초반에만(정보 과부하 완화). 빛 수치·추적 쿨다운은 상시.
    const tips = elapsed < 18;
    ctx.fillStyle = "#9fc5c5"; ctx.font = "11px 'Noto Sans KR',sans-serif";
    ctx.fillText("빛 " + Math.floor(st.light||0) + (tips ? "   [Q]비추기 · [H]직감" : ""), 132, 56);
    if (trailCD > 0) { ctx.fillStyle = "#8aa7a9"; ctx.fillText("[L] 추적  " + trailCD.toFixed(0) + "s", 26, 70); }
    else if (tips) { ctx.fillStyle = "#8ef548"; ctx.fillText("[L] 발자국 추적", 26, 70); }
    if (tips) { ctx.fillStyle = "#9fc5c5"; ctx.fillText("[Tab] 기억 일지", 26, 86); }
    ctx.restore();
  }
  ...
  function drawGuide() {
    ...
    // 일시정지 버튼(터치/마우스 공용 · ESC와 동일)
    const pb = pauseBtnRect(); ctx.save(); ctx.fillStyle = "rgba(10,16,20,.7)"; rrect(pb.x, pb.y, pb.w, pb.h, 7); ctx.fill();
    ctx.strokeStyle = "#2a4a4e"; ctx.lineWidth = 1; rrect(pb.x, pb.y, pb.w, pb.h, 7); ctx.stroke();
    ctx.fillStyle = "#8aa7a9"; ctx.fillRect(pb.x + 12, pb.y + 8, 3, 10); ctx.fillRect(pb.x + 19, pb.y + 8, 3, 10); ctx.restore();
    // 목표 배너(상단 중앙)
    ctx.save();
    ...
    if (done && boss) { const charging = boss.phase === "charge";
      ctx.fillStyle = charging ? "#ff7a7a" : "#9af6f6"; ctx.textAlign = "center";
      ctx.fillText(charging ? "⚠ 망각 파동 — 물러나라!" : "[Q] 지금! 빛으로 공백을 비춰라  (" + (boss.max - boss.hp) + "/" + boss.max + ")", W / 2, by + 22); }
  ...
  if (typeof window !== 'undefined') {
    window.__ziro = () => ({ phase, px: player.x, py: player.y, mem: st ? st.mem : 0, ch: chapterIdx,
                             camX, camY, nw: NW, nh: NH, realizeI, realizeLen: realizeLines.length, choiceSel, endingType, paused, reduceFlash });
    ...
    window.__layoutCheck = () => { const occ = (c,r) => D.collision.some(q => c>=q[0]&&c<q[0]+q[2]&&r>=q[1]&&r<q[1]+q[3]); let bad = 0;
      ...
      const reach = new Set(reachableTiles(D).map(t => t[0]+","+t[1]));   // 스폰 도달 가능 집합
      let unreachable = 0; for (const s of shards) if (!reach.has(s.tile[0]+","+s.tile[1])) unreachable++;
      return { bad, unreachable, shards: shards.length, idx: chapterIdx }; };
    window.__boss = () => boss ? { hp: boss.hp, dispelled: boss.dispelled, phase: boss.phase } : null;
    window.__shards = () => shards.map(s => ({ id: s.id, type: s.type, t: [s.tile[0], s.tile[1]] }));   // 랜덤배치 검증용
    window.__setRandom = (v) => { RANDOMIZE = !!v; };   // 테스트 결정성용(랜덤 끄기)
