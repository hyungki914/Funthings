...
(function () {
  ...
  function touchButtons() {                       // 우하단 스킬 버튼 (캔버스 px)
    ...
    return [
      ...
      { id: "q",     label: "비추기", x: bx - 98,   y: by - 8,    r: R,  hold: true },
  ...
  function update(dt) {
    ...
    for (const m of murks) {
      ...
      if (sees) { m.chasing = true; m.lost = 0;
        if (!sawMurk) { sawMurk = true; toasts.push({ text: "들켰다! [Shift] 은신으로 피하라", color: "#ff7a7a", t: 2.8 }); } }
  ...
  function draw() {
    ...
    for (const e of ents) {
      if (e.p) {
        ...
        const m = e.m;
        // 시야콘 (감지 범위 시각화 — 평시 보라, 추격 빨강)
        const hf = (m.fov * Math.PI / 180) / 2;
        ctx.save(); ctx.globalAlpha = m.chasing ? 0.18 : 0.08; ctx.fillStyle = m.chasing ? "#ff5a5a" : "#7a5cff";
        ctx.beginPath(); ctx.moveTo(m.x, m.y);
        ctx.arc(m.x, m.y, m.sightPx, m.faceAngle - hf, m.faceAngle + hf); ctx.closePath(); ctx.fill(); ctx.restore();
  ...
  function drawTouch() {
    ...
    for (const b of touchButtons()) {
      const on = (b.id === "shift" && touch.sneak) || (b.id === "q" && touch.holdQ !== null);
      ctx.save();
      ...
      ctx.fillText(b.label, b.x, b.y); ctx.textBaseline = "alphabetic"; ctx.restore();
    }
  }
